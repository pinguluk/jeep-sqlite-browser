...
export function SettingsDialog() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (wasmSource === "custom") {
        if (!customWasmUrl) {
          toast.error("Custom WASM URL is required", { duration: 3000 });
          setIsApplying(false);
          return;
        }
        if (!customScriptUrl) {
          toast.error("Custom Script URL is required", { duration: 3000 });
          setIsApplying(false);
          return;
        }
      }
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Configure extension behavior</DialogDescription>
        </DialogHeader>
          ...
          {wasmSource === "custom" && (
            <div className="space-y-3 border-t pt-4">
              <div className="flex items-start gap-2 p-2 bg-amber-500/10 border border-amber-500/20 rounded-md text-xs text-amber-600 dark:text-amber-400">
                ...
                <span>
                  Chrome MV3 restricts remote code execution. Custom CDN URLs
                  may only work in Firefox or developer mode.
                </span>
              </div>

              <div className="space-y-2">
                <Label htmlFor="customScriptUrl" className="text-sm">
                  SQL.js (sql-wasm.js) <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="customScriptUrl"
                  type="url"
                  placeholder="https://cdnjs.cloudflare.com/ajax/libs/sql.js/X.XX.X/sql-wasm.js"
              ...
              <div className="space-y-2">
                <Label htmlFor="customWasmUrl" className="text-sm">
                  WASM URL (sql-wasm.wasm) <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="customWasmUrl"
                  type="url"
                  placeholder="https://cdnjs.cloudflare.com/ajax/libs/sql.js/X.XX.X/sql-wasm.wasm"
                  value={customWasmUrl}
