...
export class DatabaseHandler {
    ...
    async init(): Promise<void> {
        ...
        try {
            ...
            this.SQL = await initSqlJs({
                locateFile: (file: string) => chrome.runtime.getURL(`sql-wasm.wasm`),
            });
    ...
    getTables(): TableInfo[] {
      ...
      `);
            ...
            const tables = result[0].values.map((row: any[]) => {
                ...
                const rowCount = countResult.length ? (countResult[0].values[0][0] as number) : 0;
     ...
     */
    updateRow(tableName: string, data: Record<string, any>, where: Record<string, any>): QueryResult {
        if (!this.db) {
            return { success: false, error: 'No database loaded' };
        }
    ...
    exportAsSQL(tableName: string | null = null): string {
        ...
        try {
            let sql = '';
            const tables = tableName ? [{ name: tableName, rowCount: 0 }] : this.getTables();
            ...
            for (const table of tables) {
                ...
                if (data.length && data[0].values.length) {
                    const columns = data[0].columns.map((c: string) => `"${c}"`).join(', ');
